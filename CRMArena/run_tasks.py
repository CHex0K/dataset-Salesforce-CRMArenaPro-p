...
def run():
    ...
    if args.task_limit and args.task_limit > 0:
        selected_tasks = dict(list(selected_tasks.items())[:args.task_limit])
        print(f"Applying task limit: {len(selected_tasks)} tasks")
    ...
    if args.agent_strategy in ["act", "react"]:
        if args.interactive:
            ...
            env = InteractiveChatEnv(tasks=selected_tasks, max_user_turns=args.max_user_turns, org_type=args.org_type, user_model=args.model, user_provider=args.llm_provider)
        else: # Not interactive, both 'act' and 'react' are fine
            env = ChatEnv(tasks=selected_tasks, org_type=args.org_type, user_model=args.model, user_provider=args.llm_provider)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help="Limit number of tasks processed (default: 5). Use 0 or negative to disable"
    )
